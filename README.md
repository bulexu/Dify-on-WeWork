...
- **CELERY_BROKER_URL**：Celery Broker 地址，默认使用 Redis。
- **WECHAT_ACCESS_TOKEN_KEY**：企业微信 access_token 在 Redis 中的缓存 key。
- **DIFY_API_URL / DIFY_API_KEY**：Dify 对话接口地址和 API Key。
- **WX_TOKEN / WX_ENCODING_AES_KEY**：企业微信回调解密参数。
- **CORP_ID / CORP_SECRET**：企业微信企业 ID 与应用 Secret。
- **WECHAT_API_URL**：企业微信发送消息接口地址。
- **APP_HOST / APP_PORT**：Uvicorn 监听地址与端口。
- **UVICORN_LOG_FILE / UVICORN_ERROR_LOG_FILE**：Uvicorn 标准输出和错误日志路径。
- **CELERY_LOG_FILE / CELERY_ERROR_LOG_FILE**：Celery 标准输出和错误日志路径。
- **REDIS_FLUSH_ON_START**：是否在应用启动时清空当前 Redis DB，默认 `false`。
...
2. **配置企业微信回调**：
    ...
    - 配置 Token 和 EncodingAESKey 与 `.env` 中的值一致。
